...
from alphasignal.database.db import create_coin, deactivate_coin, get_active_coin_balance_by_mint_address, get_active_coins, reactivate_coin, update_coin_last_price
...
def get_tracked_coins() -> List[Coin]:
...
def get_remaining_trackable_balance(mint_address: str, total_balance: float):
    """
    ...
    """
    used_balance = get_active_coin_balance_by_mint_address(mint_address)
...
async def process_coins() -> None:
    ...
    for coin in active_coins:
        current_value = jupiter_clinet.fetch_token_value(coin.mint_address)
